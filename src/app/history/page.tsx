...
function EpisodeCard({ episode, onDelete, onSelect }: {
  ...
  onSelect: (ep: SavedEpisode) => void;
}) {
  const date = new Date(episode.createdAt);
  const formattedDate = date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  const sceneCount = episode.scenes.filter((s) => s.videoUrl).length;
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-purple-400/30 transition-all group">
      ...
      <div className="aspect-video bg-gradient-to-br from-purple-900/50 to-indigo-900/50 relative overflow-hidden">
        ...
        <div className="absolute top-2 right-2 px-2 py-1 bg-black/60 rounded-lg text-xs text-white/70">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-2 text-xs text-white/40 mb-3">
          <span>{episode.childName}</span>
          <span>·</span>
          <span>{formattedDate}</span>
          <span>·</span>
          <span className="capitalize">{episode.provider}</span>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => onSelect(episode)}
            className="flex-1 py-2 bg-purple-500/20 text-purple-300 rounded-lg hover:bg-purple-500/30 text-xs font-medium transition-colors"
          >
            Watch & Download
          </button>
          <button
...
function EpisodeViewer({ episode, onBack }: { episode: SavedEpisode; onBack: () => void }) {
  ...
  const scenesWithVideo = episode.scenes.filter((s) => s.videoUrl);
  const scene = scenesWithVideo[currentScene];

  if (!scene) {
    return (
      <div className="text-center py-20">
        <p className="text-white/50">No video scenes in this episode.</p>
  ...
  const playScene = () => {
    ...
    if (audioRef.current && scene.audioUrl) { audioRef.current.currentTime = 0; audioRef.current.play(); }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <button onClick={onBack} className="mb-4 text-sm text-white/40 hover:text-white/70 flex items-center gap-1 transition-colors">
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
        <div className="relative aspect-video bg-black flex items-center justify-center">
          <video
            ...
            playsInline
          />
          {scene.audioUrl && <audio ref={audioRef} key={scene.audioUrl} src={scene.audioUrl} />}
          {!isPlaying && (
            <button onClick={playScene} className="absolute inset-0 flex items-center justify-center bg-black/40">
              <div className="w-16 h-16 bg-white/20 backdrop-blur-xl rounded-full flex items-center justify-center hover:bg-white/30 transition-all">
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-purple-400 text-sm">Scene {currentScene + 1} / {scenesWithVideo.length}</span>
          </div>
          ...
          <div className="flex gap-2 justify-center flex-wrap">
            {scene.videoUrl && (
              <button onClick={() => downloadFile(scene.videoUrl!, `${safeTitle}_scene_${currentScene + 1}.mp4`)}
            ...
            {scene.audioUrl && (
              <button onClick={() => downloadFile(scene.audioUrl!, `${safeTitle}_narration_${currentScene + 1}.mp3`)}
            ...
            <button
              onClick={async () => {
                for (let i = 0; i < scenesWithVideo.length; i++) {
                  ...
                  if (s.videoUrl) await downloadFile(s.videoUrl, `${safeTitle}_scene_${i + 1}.mp4`);
                  if (s.audioUrl) await downloadFile(s.audioUrl, `${safeTitle}_narration_${i + 1}.mp3`);
                  await new Promise((r) => setTimeout(r, 500));
                }
              }}
              className="px-4 py-2 bg-purple-500/20 text-purple-300 rounded-lg hover:bg-purple-500/30 text-sm transition-colors flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
              Download All
            </button>
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-950 via-purple-950 to-slate-950 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/create" className="px-5 py-2.5 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold rounded-xl hover:from-purple-600 hover:to-pink-600 transition-all text-sm">
            Create New
          </Link>
        </div>
        ...
        {selectedEpisode ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-white/50 mb-6">Generated episodes will appear here after you save them.</p>
            <Link href="/create" className="px-6 py-3 bg-purple-500/20 text-purple-300 rounded-xl hover:bg-purple-500/30 transition-colors">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {episodes.map((ep) => (
              <EpisodeCard key={ep.id} episode={ep} onDelete={handleDelete} onSelect={setSelectedEpisode} />
            ))}
          </div>
        )}
      </div>
